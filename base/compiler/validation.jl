...
function _validate_val!(@nospecialize(x), errors, ssavals::BitSet)
    if isa(x, Expr)
        if x.head === :call || x.head === :invoke || x.head === :invoke_modifyfield!
            f = x.args[1]
    ...
    for x in c.code
        if isa(x, Expr)
            ...
            elseif head === :(=)
                ...
                validate_val!(lhs)
                validate_val!(rhs)
            elseif head === :call || head === :invoke || x.head === :invoke_modifyfield! ||
...
function is_valid_rvalue(@nospecialize(x))
    ...
    if isa(x, Expr) && x.head in (:new, :splatnew, :the_exception, :isdefined, :call, :invoke, :invoke_modifyfield!, :foreigncall, :cfunction, :gc_preserve_begin, :copyast)
        return true
    end
    return false
end
